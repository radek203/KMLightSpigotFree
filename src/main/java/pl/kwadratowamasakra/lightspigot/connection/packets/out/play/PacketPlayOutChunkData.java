...
public class PacketPlayOutChunkData extends PacketOut {
    ...
    private byte[] convertLegacyDataToModernFormat(final byte[] legacyData) {
        ...
        final int[][] blockStates = new int[sections][SECTION_BLOCK_COUNT];
        for (int sectionIndex = 0; sectionIndex < sections; sectionIndex++) {
            for (int blockIndex = 0; blockIndex < SECTION_BLOCK_COUNT; blockIndex++) {
                blockStates[sectionIndex][blockIndex] = legacyBuffer.readUnsignedByte() | legacyBuffer.readUnsignedByte() << 8;
            }
        }

        final byte[][] blockLights = new byte[sections][LIGHT_BYTES];
        for (int sectionIndex = 0; sectionIndex < sections; sectionIndex++) {
            legacyBuffer.readBytes(blockLights[sectionIndex]);
        }

        final byte[][] skyLights = new byte[sections][LIGHT_BYTES];
        for (int sectionIndex = 0; sectionIndex < sections; sectionIndex++) {
            legacyBuffer.readBytes(skyLights[sectionIndex]);
        }

        for (int sectionIndex = 0; sectionIndex < sections; sectionIndex++) {
            writeModernSection(modernBuffer, blockStates[sectionIndex], blockLights[sectionIndex], skyLights[sectionIndex]);
        }
    ...
    private void writeModernSection(final PacketBuffer modernBuffer, final int[] blockStates,
                                    final byte[] blockLight, final byte[] skyLight) {
